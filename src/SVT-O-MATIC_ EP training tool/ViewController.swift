...
class ViewController: UIViewController, WKNavigationDelegate, UIDocumentInteractionControllerDelegate {
    ...
    override func viewDidLayoutSubviews() {
        ...
        SVT_O_MATIC_EPtrainingtool.webView.frame = calcWebviewFrame(webviewView: webviewView, toolbarView: nil)
    }
    
    @objc func keyboardWillHide(_ notification: NSNotification) {
        SVT_O_MATIC_EPtrainingtool.webView.setNeedsLayout()
    }
    
    func initWebView() {
        SVT_O_MATIC_EPtrainingtool.webView = createWebView(container: webviewView, WKSMH: self, WKND: self, NSO: self, VC: self)
        webviewView.addSubview(SVT_O_MATIC_EPtrainingtool.webView);
        
        SVT_O_MATIC_EPtrainingtool.webView.uiDelegate = self;
        
        SVT_O_MATIC_EPtrainingtool.webView.addObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress), options: .new, context: nil)

        if(pullToRefresh){
            let refreshControl = UIRefreshControl()
            refreshControl.addTarget(self, action: #selector(refreshWebView(_:)), for: UIControl.Event.valueChanged)
            SVT_O_MATIC_EPtrainingtool.webView.scrollView.addSubview(refreshControl)
            SVT_O_MATIC_EPtrainingtool.webView.scrollView.bounces = true
        }

        if #available(iOS 15.0, *), adaptiveUIStyle {
            themeObservation = SVT_O_MATIC_EPtrainingtool.webView.observe(\.themeColor) { [unowned self] webView, _ in
                let backgroundColor = SVT_O_MATIC_EPtrainingtool.webView.underPageBackgroundColor;
                let themeColor = SVT_O_MATIC_EPtrainingtool.webView.themeColor;
    ...
    @objc func refreshWebView(_ sender: UIRefreshControl) {
        SVT_O_MATIC_EPtrainingtool.webView?.reload()
...
//        toolbarView.autoresizingMask = [.flexibleTopMargin, .flexibleRightMargin, .flexibleWidth]
    ...
    func overrideUIStyle(toDefault: Bool = false) {
        if #available(iOS 15.0, *), adaptiveUIStyle {
            if (((htmlIsLoaded && !SVT_O_MATIC_EPtrainingtool.webView.isHidden) || toDefault) && self.currentWebViewTheme != .unspecified) {
                UIApplication
                    .shared
                    .connectedScenes
    ...
    @objc func loadRootUrl(cachePolicy: NSURLRequest.CachePolicy = .useProtocolCachePolicy) {
        SVT_O_MATIC_EPtrainingtool.webView.load(URLRequest(url: SceneDelegate.universalLinkToLaunch ?? SceneDelegate.shortcutLinkToLaunch ?? rootUrl, cachePolicy: cachePolicy))
    }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!){
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
            SVT_O_MATIC_EPtrainingtool.webView.isHidden = false
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if (keyPath == #keyPath(WKWebView.estimatedProgress) &&
                SVT_O_MATIC_EPtrainingtool.webView.isLoading &&
                !self.loadingView.isHidden &&
                !self.htmlIsLoaded) {
                    var progress = Float(SVT_O_MATIC_EPtrainingtool.webView.estimatedProgress);
    ...
    deinit {
        SVT_O_MATIC_EPtrainingtool.webView.removeObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress))
    }
}
...
extension ViewController: WKScriptMessageHandler {
  func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if message.name == "print" {
            printView(webView: SVT_O_MATIC_EPtrainingtool.webView)
        }
